...
import webbrowser
import sys
import multiprocessing  # Importado temprano para contar CPUs
...
# Obtener número de CPUs y configurar PyTorch primero
thread_count = multiprocessing.cpu_count()

# Importar torch y configurar hilos inmediatamente
import torch
...
# Resto de imports después de configurar PyTorch
import torchaudio
import random
import numpy as np
...
#Agregar text_tokenizer
text_tokenizer = PhonemeBpeTokenizer(tokenizer_path="./utils/g2p/bpe_69.json")
text_collater = get_text_token_collater()

# Configuración del dispositivo después de imports
...
# Carga del modelo Whisper (ajusta el tamaño que necesites, ej. "base", "small", etc.)
whisper_model = whisper.load_model("base")
whisper_model.to(device)
# Directorio donde se guardarán los audios generados
output_dir = "output"
os.makedirs(output_dir, exist_ok=True)
# Montar la carpeta "output" para servir archivos estáticos (audio)
...
async def infer_audio_endpoint(
    ...
    try:
        ...
        accent = "no-accent"
        transcript_content = ""
        # Seleccionamos el archivo de audio a utilizar:
        # Si se sube uno mediante upload_audio_prompt, lo usamos;
        # de lo contrario, usamos record_audio_prompt.
        audio_file = upload_audio_prompt if upload_audio_prompt is not None else record_audio_prompt
        ...
        # Leer el archivo de audio con soundfile para obtener la señal y la tasa de muestreo
        wav, sr = sf.read(temp_path, dtype="float32")
        # Si el audio es estéreo, lo convertimos a mono (usamos el primer canal)
        if wav.ndim == 2 and wav.shape[1] > 1:
            wav = wav[:, 0]
        audio_tuple = (sr, wav)
        
        # Eliminamos el archivo temporal de entrada
        os.remove(temp_path)
        logging.info("Archivo temporal de entrada eliminado.")
        
        # Llamar a tu función de inferencia (record_audio_prompt se pasa como None si ya se usó upload_audio_prompt)
        text_output, audio_out_tuple = infer_from_audio(
            "",
            language,
            accent,
            audio_tuple,
            None,  # record_audio_prompt: no lo usamos si ya tenemos upload_audio_prompt
            transcript_content
        )
        out_sr, out_audio = audio_out_tuple
        
        # Guardar el audio de salida en un archivo WAV para poder escucharlo.
        # Se usa un nombre único con timestamp.
        ...
        # Se retorna un JSON con el mensaje generado y la URL para escuchar el audio.
        # La URL será del estilo: http://127.0.0.1:8000/audio/generated_<timestamp>.wav
